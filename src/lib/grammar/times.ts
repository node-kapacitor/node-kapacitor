...
class NanoDate extends Date{ 
  ...
  constructor(opt: { nanoTime?: string, nanoISO?: string }) {
    super();
    opt.nanoISO ? this.nanoISO = opt.nanoISO : null;
    opt.nanoTime ? this.nanoTime = opt.nanoTime : null;
  }

  getNanoTimeFromISO() {
    if (!this.cachedNanoISO) {
      this.cachedNanoTime = nanoIsoToTime(this.nanoISO);
    }
    return this.cachedNanoTime;
  }
  ...
  toNanoISOStringFromISO(): string {
    ...
    return `${base}${this.cachedNanoTime.slice(-9)}Z`;
  }
  ...
  toNanoISOStringFromStamp(): string {
    const base = this.toISOString().slice(0, -4); // slice of `123Z` milliseconds
    return `${base}${this.nanoTime.slice(-9)}Z`;
  }
}
...
export function toNanoDate(timestamp: string): INanoDate {
  ...
  const nanoDate = new NanoDate({ nanoTime: date._nanoTime });
  date.getNanoTime = nanoDate.getNanoTimeFromStamp;
  date.toNanoISOString = nanoDate.toNanoISOStringFromStamp;
  return date;
}

function asNanoDate(date: Date): INanoDate | undefined {
  const d = <any> date;
  if (d.getNanoTime) {
    return d;
  }
...
class NanosecondsDateManipulator implements IDateManipulator<INanoDate> {
  ...
  public isoToDate(timestamp: string): INanoDate {
    ...
    const nanoDate = new NanoDate({ nanoISO: date._nanoISO });
    date.getNanoTime = nanoDate.getNanoTimeFromISO;
    date.toNanoISOString = nanoDate.toNanoISOStringFromISO;
    return date;
  }
